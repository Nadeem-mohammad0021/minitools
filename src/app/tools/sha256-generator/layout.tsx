...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'sha256-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Sha256 Generator';
  const description = tool ? tool.description : 'Free online Sha256 Generator tool. Fast, secure, and easy to use.';
