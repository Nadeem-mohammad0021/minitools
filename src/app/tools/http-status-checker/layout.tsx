...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'http-status-checker';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Http Status Checker';
  const description = tool ? tool.description : 'Free online Http Status Checker tool. Fast, secure, and easy to use.';
