...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'remove-pdf-restrictions';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Remove Pdf Restrictions';
  const description = tool ? tool.description : 'Free online Remove Pdf Restrictions tool. Fast, secure, and easy to use.';
