...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'paragraph-counter';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Paragraph Counter';
  const description = tool ? tool.description : 'Free online Paragraph Counter tool. Fast, secure, and easy to use.';
