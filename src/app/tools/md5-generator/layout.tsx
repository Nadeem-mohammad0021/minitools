...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'md5-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Md5 Generator';
  const description = tool ? tool.description : 'Free online Md5 Generator tool. Fast, secure, and easy to use.';
