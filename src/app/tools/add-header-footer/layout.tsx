...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'add-header-footer';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Add Header Footer';
  const description = tool ? tool.description : 'Free online Add Header Footer tool. Fast, secure, and easy to use.';
