...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'meta-tags-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Meta Tags Generator';
  const description = tool ? tool.description : 'Free online Meta Tags Generator tool. Fast, secure, and easy to use.';
