...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'js-minifier';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Js Minifier';
  const description = tool ? tool.description : 'Free online Js Minifier tool. Fast, secure, and easy to use.';
