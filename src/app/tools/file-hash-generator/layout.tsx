...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'file-hash-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'File Hash Generator';
  const description = tool ? tool.description : 'Free online File Hash Generator tool. Fast, secure, and easy to use.';
