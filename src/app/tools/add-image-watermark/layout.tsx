...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'add-image-watermark';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Add Image Watermark';
  const description = tool ? tool.description : 'Free online Add Image Watermark tool. Fast, secure, and easy to use.';
