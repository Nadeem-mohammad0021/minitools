...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'remove-line-breaks';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Remove Line Breaks';
  const description = tool ? tool.description : 'Free online Remove Line Breaks tool. Fast, secure, and easy to use.';
