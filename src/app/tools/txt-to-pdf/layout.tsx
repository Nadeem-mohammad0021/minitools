...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'txt-to-pdf';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Txt To Pdf';
  const description = tool ? tool.description : 'Free online Txt To Pdf tool. Fast, secure, and easy to use.';
