...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'css-minifier';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Css Minifier';
  const description = tool ? tool.description : 'Free online Css Minifier tool. Fast, secure, and easy to use.';
