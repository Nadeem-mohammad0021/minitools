...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'sha1-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Sha1 Generator';
  const description = tool ? tool.description : 'Free online Sha1 Generator tool. Fast, secure, and easy to use.';
