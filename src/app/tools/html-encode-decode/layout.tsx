...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'html-encode-decode';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Html Encode Decode';
  const description = tool ? tool.description : 'Free online Html Encode Decode tool. Fast, secure, and easy to use.';
