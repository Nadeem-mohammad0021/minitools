...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'remove-duplicates';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Remove Duplicates';
  const description = tool ? tool.description : 'Free online Remove Duplicates tool. Fast, secure, and easy to use.';
