...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'url-slug-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Url Slug Generator';
  const description = tool ? tool.description : 'Free online Url Slug Generator tool. Fast, secure, and easy to use.';
