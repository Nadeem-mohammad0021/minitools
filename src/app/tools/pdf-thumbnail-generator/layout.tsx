...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'pdf-thumbnail-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Pdf Thumbnail Generator';
  const description = tool ? tool.description : 'Free online Pdf Thumbnail Generator tool. Fast, secure, and easy to use.';
