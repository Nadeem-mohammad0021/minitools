...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'unzip-files';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Unzip Files';
  const description = tool ? tool.description : 'Free online Unzip Files tool. Fast, secure, and easy to use.';
