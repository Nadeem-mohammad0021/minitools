...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'pdf-metadata-viewer';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Pdf Metadata Viewer';
  const description = tool ? tool.description : 'Free online Pdf Metadata Viewer tool. Fast, secure, and easy to use.';
