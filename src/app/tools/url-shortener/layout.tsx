...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'url-shortener';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Url Shortener';
  const description = tool ? tool.description : 'Free online Url Shortener tool. Fast, secure, and easy to use.';
