...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'xml-sitemap-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Xml Sitemap Generator';
  const description = tool ? tool.description : 'Free online Xml Sitemap Generator tool. Fast, secure, and easy to use.';
