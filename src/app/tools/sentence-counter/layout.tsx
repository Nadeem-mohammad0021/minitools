...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'sentence-counter';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Sentence Counter';
  const description = tool ? tool.description : 'Free online Sentence Counter tool. Fast, secure, and easy to use.';
