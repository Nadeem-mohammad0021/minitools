...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'find-and-replace-text';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Find And Replace Text';
  const description = tool ? tool.description : 'Free online Find And Replace Text tool. Fast, secure, and easy to use.';
