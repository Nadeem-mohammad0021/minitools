...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'convert-csv-to-excel';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Convert Csv To Excel';
  const description = tool ? tool.description : 'Free online Convert Csv To Excel tool. Fast, secure, and easy to use.';
