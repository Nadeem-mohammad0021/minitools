...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'seo-page-analyzer-lite';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Seo Page Analyzer Lite';
  const description = tool ? tool.description : 'Free online Seo Page Analyzer Lite tool. Fast, secure, and easy to use.';
