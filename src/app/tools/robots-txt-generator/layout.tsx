...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'robots-txt-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Robots Txt Generator';
  const description = tool ? tool.description : 'Free online Robots Txt Generator tool. Fast, secure, and easy to use.';
