...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'sitemap-validator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Sitemap Validator';
  const description = tool ? tool.description : 'Free online Sitemap Validator tool. Fast, secure, and easy to use.';
