...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'robots-txt-tester';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Robots Txt Tester';
  const description = tool ? tool.description : 'Free online Robots Txt Tester tool. Fast, secure, and easy to use.';
