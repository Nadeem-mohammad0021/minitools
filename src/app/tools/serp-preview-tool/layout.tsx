...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'serp-preview-tool';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Serp Preview Tool';
  const description = tool ? tool.description : 'Free online Serp Preview Tool tool. Fast, secure, and easy to use.';
