...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'random-text-generator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Random Text Generator';
  const description = tool ? tool.description : 'Free online Random Text Generator tool. Fast, secure, and easy to use.';
