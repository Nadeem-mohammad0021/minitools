...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'url-checker';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Url Checker';
  const description = tool ? tool.description : 'Free online Url Checker tool. Fast, secure, and easy to use.';
