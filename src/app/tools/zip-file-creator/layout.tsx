...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'zip-file-creator';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Zip File Creator';
  const description = tool ? tool.description : 'Free online Zip File Creator tool. Fast, secure, and easy to use.';
