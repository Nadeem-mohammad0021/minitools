...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'excel-to-csv';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Excel To Csv';
  const description = tool ? tool.description : 'Free online Excel To Csv tool. Fast, secure, and easy to use.';
