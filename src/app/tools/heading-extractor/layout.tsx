...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'heading-extractor';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Heading Extractor';
  const description = tool ? tool.description : 'Free online Heading Extractor tool. Fast, secure, and easy to use.';
