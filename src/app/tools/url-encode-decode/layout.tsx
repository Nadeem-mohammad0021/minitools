...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'url-encode-decode';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Url Encode Decode';
  const description = tool ? tool.description : 'Free online Url Encode Decode tool. Fast, secure, and easy to use.';
