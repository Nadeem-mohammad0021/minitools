...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'pdf-password';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Pdf Password';
  const description = tool ? tool.description : 'Free online Pdf Password tool. Fast, secure, and easy to use.';
