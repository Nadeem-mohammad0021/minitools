...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'pdf-to-txt';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Pdf To Txt';
  const description = tool ? tool.description : 'Free online Pdf To Txt tool. Fast, secure, and easy to use.';
