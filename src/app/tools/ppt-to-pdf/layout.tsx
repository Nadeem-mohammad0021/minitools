...
export async function generateMetadata({ params }: { params: any }): Promise<Metadata> {
  const toolId = 'ppt-to-pdf';
  const tool = TOOLS.find(t => t.id === toolId || t.path.includes(toolId));
  
  const title = tool ? tool.name : 'Ppt To Pdf';
  const description = tool ? tool.description : 'Free online Ppt To Pdf tool. Fast, secure, and easy to use.';
